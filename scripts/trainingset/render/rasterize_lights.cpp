...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  //declare variables
  mxArray *a_in_m, *b_in_m, *c_in_m, *d_in_m, *e_in_m, *f_in_m, *g_in_v, *h_in_m, *i_in_m, *j_in_m, *k_in_s, *l_in_s, *m_in_v, *n_out_m;
  double *triangle_list, *vertices, *color, *color_m, *l_amb, *l_dir, *light, *view_m, *reflect_m, *normal_m, *ks, *vu, *resolution, *frame_buffer;
  ...
  m_in_v = mxDuplicateArray(prhs[12]);
  ...
  ks = mxGetPr(k_in_s);
  vu = mxGetPr(l_in_s);
  resolution = mxGetPr(m_in_v);
  
  //associate outputs
  mwSize dims1[3] = {(int)resolution[0],(int)resolution[1], 3};
  n_out_m = plhs[0] = mxCreateNumericArray(3, dims1, mxDOUBLE_CLASS, mxREAL);
  
  //associate pointers to the data in the output mxArray
  frame_buffer = mxGetPr(n_out_m);
          ...
          if (z_buffer[y+x*height] > z) {     
           ...
           n_l_dot = normal[0] * light[0] + normal[1] * light[1] + normal[2] * light[2];
            ...
            if (n_l_dot > 0) {
              ...
              i_color[0] = i_color[0] + diffuse[0];
              i_color[1] = i_color[1] + diffuse[1];
              i_color[2] = i_color[2] + diffuse[2];
              
              if (r_v_dot > 0) {
              
                specular[0] = (1 - l_dir[0]) * ks[0] * pow(r_v_dot, vu[0]);
                specular[1] = (1 - l_dir[5]) * ks[0] * pow(r_v_dot, vu[0]);
                specular[2] = (1 - l_dir[10]) * ks[0] * pow(r_v_dot, vu[0]);

                i_color[0] = i_color[0] + specular[0];
                i_color[1] = i_color[1] + specular[1];
                i_color[2] = i_color[2] + specular[2];
              }
            }
